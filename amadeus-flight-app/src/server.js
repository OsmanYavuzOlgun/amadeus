...
const flights = [
  {
    ...
    arrivalTime: "2024-01-01T14:15:00",
...
app.post("/api/searchFlights", (req, res) => {
  ...
  let filteredFlights = flights.filter(
    (flight) => flight.departure === departure && flight.arrival === arrival && new Date(flight.departureTime).toDateString() === new Date(departureDate).toDateString()  );

  if (!isOneWay) {
    const filteredReturnFlights = returnFlights.filter(
      (flight) => flight.departure === arrival && flight.arrival === departure && new Date(flight.departureTime).toDateString() === new Date(returnDate).toDateString() );
